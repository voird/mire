...
need_to(goblin, gold_coin).
need_to(gnome, wooden_staff).
need_to(dwarf, armor_shield ).
need_to(knight, steel_sword).
need_to(wizard, ancient_scroll).
...
give(Bot, Item) :-
    ...
    (member(Item, Inv) ->
        ...
        retract(inventory(Inv)),
        assert(inventory(NewInv)),
        format('You gave ~w to ~w.~n', [Item, Bot])
    ;
        write('You do not have this item.'), nl
    ).
